...
public class AccentColorNameToBrushConverter : IValueConverter {
    ...
    public static readonly AccentColorNameToBrushConverter Instance = new();
    
    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture) {
        if (value is not string name) return null;
        return AccentColorsBase.AccentColors
                   .FirstOrDefault(c => c.Name == name)?.PrimaryColor
               ?? AccentColorsBase.AccentColors.First().PrimaryColor;
    }
